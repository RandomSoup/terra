...
tr_always_inline void rover_set_status(rover_t* rover)
{
	uint8_t type;
...
tr_always_inline void rover_scroll(rover_t* rover, int key)
{
	size_t sz;
	size_t off;
	surf_t* surf;
...
void rover_click_cb(int button, int x, int y, void* udata)
{
	...
	for (ssize_t i = 0; i < links->len; i++)
	{
		...
		if (link->line == row)
		{
			...
			rover->url = piper_path_resolve(rover->piper, link->url, false);
			rover->pending = true;
			break;
		}
	}
